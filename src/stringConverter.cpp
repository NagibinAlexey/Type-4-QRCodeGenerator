...
namespace QR {
    ...
    stringConverter::stringConverter(std::string string, int version, ErrorCorLevel level) : data_(std::move(string)) {
    ...
    std::string stringConverter::generateFullBitString() {
    ...
    std::string stringConverter::generateBinaryData() {
        ...
        else if (info_.mode == QR::Byte) {
            return byteToBitString(data_);
        }
        return byteToBitString(data_);
    }

    int stringConverter::calculateCapacity() {
    ...
    QRGeneratorInfo stringConverter::getQRInfo() const {
        return info_;
    }

    std::string stringConverter::getFullBitString() {
